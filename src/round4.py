...
class Trader:
    ...
    def orchids_adap_edge(
        ...
        if timestamp == 0:
            self.orchids_data["curr_edge"] = self.params["MAGNIFICENT_MACARONS"]["init_make_edge"]
            return self.params["MAGNIFICENT_MACARONS"]["init_make_edge"]
    
        # Timestamp not 0
        self.orchids_data["volume_history"].append(abs(position))
        if len(self.orchids_data["volume_history"]) > self.params["MAGNIFICENT_MACARONS"]["volume_avg_timestamp"]:
        ...
        if len(self.orchids_data["volume_history"]) < self.params["MAGNIFICENT_MACARONS"]["volume_avg_timestamp"]:
            ...
            if volume_avg >= self.params["MAGNIFICENT_MACARONS"]["volume_bar"]:
                self.orchids_data["volume_history"] = [] # clear volume history if edge changed
                self.orchids_data["curr_edge"] = curr_edge + self.params["MAGNIFICENT_MACARONS"]["step_size"]
                return curr_edge + self.params["MAGNIFICENT_MACARONS"]["step_size"]
            
            # Decrement edge if more cash with less edge, included discount
            elif self.params["MAGNIFICENT_MACARONS"]["dec_edge_discount"] * self.params["MAGNIFICENT_MACARONS"]["volume_bar"] * (curr_edge - self.params["MAGNIFICENT_MACARONS"]["step_size"]) > volume_avg * curr_edge:
                if curr_edge - self.params["MAGNIFICENT_MACARONS"]["step_size"] > self.params["MAGNIFICENT_MACARONS"]["min_edge"]:
                    self.orchids_data["volume_history"] = [] # clear volume history if edge changed
                    self.orchids_data["curr_edge"] = curr_edge - self.params["MAGNIFICENT_MACARONS"]["step_size"]
                    self.orchids_data["optimized"] = True
                    return curr_edge - self.params["MAGNIFICENT_MACARONS"]["step_size"]
                else:
                    self.orchids_data["curr_edge"] = self.params["MAGNIFICENT_MACARONS"]["min_edge"]
                    return self.params["MAGNIFICENT_MACARONS"]["min_edge"]
    ...
    def orchids_arb_take(
        ...
        orders: List[Order] = []
        position_limit = self.limits["MAGNIFICENT_MACARONS"]
        buy_order_volume = 0
        sell_order_volume = 0
        ...
        edge = (ask - implied_ask) * self.params["MAGNIFICENT_MACARONS"]["make_probability"]
        ...
        for price in sorted(list(order_depth.sell_orders.keys())):
            ...
            if price < implied_bid - edge:
                ...
                if quantity > 0:
                    orders.append(Order("MAGNIFICENT_MACARONS", round(price), quantity))
                    buy_order_volume += quantity
    ...
    def orchids_arb_make(
        ...
        if buy_quantity > 0:
            orders.append(Order("MAGNIFICENT_MACARONS", round(bid), buy_quantity))  # Buy order

        sell_quantity = position_limit + (position - sell_order_volume)
        if sell_quantity > 0:
            orders.append(Order("MAGNIFICENT_MACARONS", round(ask), -sell_quantity))  # Sell order
    ...
    # ) -> float: 
    #     if timestamp == 0:
    #         trader_data["ORCHIDS"]["curr_edge"] = self.params["MAGNIFICENT_MACARONS"]["init_make_edge"]
    #         return self.params["MAGNIFICENT_MACARONS"]["init_make_edge"]

    #     # Timestamp not 0
    #     trader_data["ORCHIDS"]["volume_history"].append(abs(position))
    #     if len(trader_data["ORCHIDS"]["volume_history"]) > self.params["MAGNIFICENT_MACARONS"]["volume_avg_timestamp"]:
    #         trader_data["ORCHIDS"]["volume_history"].pop(0)

    #     if len(trader_data["ORCHIDS"]["volume_history"]) < self.params["MAGNIFICENT_MACARONS"]["volume_avg_timestamp"]:
    #         return curr_edge
    #     elif not trader_data["ORCHIDS"]["optimized"]:
    #         volume_avg = np.mean(trader_data["ORCHIDS"]["volume_history"])

    #         # Bump up edge if consistently getting lifted full size
    #         if volume_avg >= self.params["MAGNIFICENT_MACARONS"]["volume_bar"]:
    #             trader_data["ORCHIDS"]["volume_history"] = [] # clear volume history if edge changed
    #             trader_data["ORCHIDS"]["curr_edge"] = curr_edge + self.params["MAGNIFICENT_MACARONS"]["step_size"]
    #             return curr_edge + self.params["MAGNIFICENT_MACARONS"]["step_size"]
    ...
    #                 trader_data["ORCHIDS"]["volume_history"] = [] # clear volume history if edge changed
    #                 trader_data["ORCHIDS"]["curr_edge"] = curr_edge - self.params["MAGNIFICENT_MACARONS"]["step_size"]
    #                 trader_data["ORCHIDS"]["optimized"] = True
    #                 return curr_edge - self.params["MAGNIFICENT_MACARONS"]["step_size"]
    #             else:
    #                 trader_data["ORCHIDS"]["curr_edge"] = self.params["MAGNIFICENT_MACARONS"]["min_edge"]
    #                 return self.params["MAGNIFICENT_MACARONS"]["min_edge"]

    #     trader_data["ORCHIDS"]["curr_edge"] = curr_edge
    #     return curr_edge
    ...
    #     # don't lose money
    #     if aggressive_ask >= implied_ask + self.params["MAGNIFICENT_MACARONS"]['min_edge']:
    #         ask = aggressive_ask
    #         print("AGGRESSIVE")
    #         print(f"ALGO ASK: {round(ask)}")
    #         print(f"ALGO BID: {round(bid)}")
    ...
    def run(self, state : TradingState) -> tuple[dict[Symbol, list[Order]], int , str]:
        ...
        if "MAGNIFICENT_MACARONS" in self.params and "MAGNIFICENT_MACARONS" in state.order_depths:
            orchids_position = (
            ...
            orchids_take_orders, buy_order_volume, sell_order_volume = (
                self.take_orders(
                    ...
                    state.order_depths["MAGNIFICENT_MACARONS"],
                    orchids_fair_value,
                    self.params["MAGNIFICENT_MACARONS"]["take_width"],
                    orchids_position
                )
            )
            orchids_clear_orders, buy_order_volume, sell_order_volume = (
                self.clear_orders(
                    ...
                    state.order_depths["MAGNIFICENT_MACARONS"],
                    orchids_fair_value,
                    self.params["MAGNIFICENT_MACARONS"]["clear_width"],
                    orchids_position,
                    buy_order_volume,
                    sell_order_volume,
                )
            ...
            orchids_make_orders, _, _ = self.make_orchids_orders(
                state.order_depths["MAGNIFICENT_MACARONS"],
                orchids_fair_value,
                self.params["MAGNIFICENT_MACARONS"]["make_min_edge"],
                orchids_position,
                buy_order_volume,
                sell_order_volume,
            )
        ...
        # if "MAGNIFICENT_MACARONS" in self.params and "MAGNIFICENT_MACARONS" in state.order_depths:
        #     if "ORCHIDS" not in trader_data:
        #         trader_data["ORCHIDS"] = {"curr_edge": self.params["MAGNIFICENT_MACARONS"]["init_make_edge"], "volume_history": [], "optimized": False}
        #     orchids_position = (
        #         state.position["MAGNIFICENT_MACARONS"]
        #         if "MAGNIFICENT_MACARONS" in state.position
        #         else 0
        ...
        #         else 0
        #     )
        #     print(f"MACARONS POSITION: {orchids_position}")
        ...
        #     orders["MAGNIFICENT_MACARONS"] = orchids_take_orders + orchids_make_orders
