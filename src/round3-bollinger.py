...
class BaseVolcanicRockVoucherStrategy(MarketMakeStrategy):
    def __init__(self, symbol: str, limit: int, order_depth: OrderDepth, trader_data):
        ...
        self.risk_free_rate = 0.01  # Assumed 1% risk-free rate
        self.take_width = 2
        self.clear_width = 1
        self.disregard_edge = 1
        self.join_edge = 0
    ...
    def fair_price(self, state: TradingState) -> float:
        """Calculate fair price for the voucher using Black-Scholes model"""
        ...
        if len(self.order_depth.buy_orders) > 0 and len(self.order_depth.sell_orders) > 0:
            ...
            implied_vol = self.estimate_implied_volatility(
                market_price, spot_price, self.strike_price, time_to_expiry, self.risk_free_rate
            )
        ...
        # Adjust fair value based on time decay and market conditions
        if option_price < 0.5 and self.strike_price > spot_price:
            # For deep OTM options nearing expiry, be more conservative
            option_price = max(0.5, option_price * 0.8)
        ...
        delta = self.calculate_delta(spot_price, self.strike_price, time_to_expiry, self.risk_free_rate, self.volatility_estimate)
        theta = self.calculate_theta(spot_price, self.strike_price, time_to_expiry, self.risk_free_rate, self.volatility_estimate)
        gamma = self.calculate_gamma(spot_price, self.strike_price, time_to_expiry, self.risk_free_rate, self.volatility_estimate)
        ...
        # Set appropriate edge values based on option characteristics
        if self.days_to_expiry <= 3:
            # For options close to expiry, tighten spreads
            self.take_width = 1
            self.default_edge = 1
            self.disregard_edge = 0
        elif abs(spot_price - self.strike_price) < 300:
            # For ATM options, tighter control
            self.take_width = 2
            self.default_edge = 2
            self.disregard_edge = 1
        else:
            # For far OTM or ITM options, wider spreads
            self.take_width = 3
            self.default_edge = 3
    ...
    def calculate_delta(self, spot_price, strike, time_to_expiry, risk_free_rate, volatility):
    ...
    def calculate_theta(self, spot_price, strike, time_to_expiry, risk_free_rate, volatility):
    ...
    def act(self, state: TradingState):
        ...
        # Get volcanic rock position for delta hedging
        rock_position = state.position.get("VOLCANIC_ROCK", 0)
        
        # Get Bollinger Bands info
        lower_band = self.trader_data.get(f"{self.symbol}_bb_lower", None)
        sma = self.trader_data.get(f"{self.symbol}_bb_sma", None)
        upper_band = self.trader_data.get(f"{self.symbol}_bb_upper", None)
        ...
        # Adjust strategy based on Bollinger Bands
        if lower_band and upper_band and sma and current_price:
            # If price is above upper band, be more aggressive selling
            if current_price > upper_band:
                self.take_width = max(0, self.take_width - 1)
                self.fair_value = min(self.fair_value, current_price * 0.99)
            # If price is below lower band, be more aggressive buying
            elif current_price < lower_band:
                self.take_width = max(0, self.take_width - 1)
                self.fair_value = max(self.fair_value, current_price * 1.01)
                
        # Adjust position limits based on delta exposure
        delta = self.trader_data.get(f"{self.symbol}_delta", 0.5)
        moneyness = self.trader_data.get(f"{self.symbol}_moneyness", 1.0)
        
        # Apply object-specific parameter adjustments
        self.adjust_parameters(delta, moneyness)
        
        # If we have significant theta exposure, adjust our pricing to account for time decay
        theta = self.trader_data.get(f"{self.symbol}_theta", 0)
        if abs(theta) > 5 and self.days_to_expiry <= 3:
            # If significant time decay and close to expiration, adjust fair value
            self.fair_value *= 0.97
        
        # Run standard market making logic with our adjusted parameters
        buy_order_volume = 0
        sell_order_volume = 0
        
        # Taking orders at favorable prices
        take, buy_order_volume, sell_order_volume = self.take_orders(position, buy_order_volume, sell_order_volume)
        
        # Clear positions as needed
        clear, buy_order_volume, sell_order_volume = self.clear_orders(position, buy_order_volume, sell_order_volume)
        
        # Make standard orders
        make, _, _ = self.make_orders(position, buy_order_volume, sell_order_volume)
        
        # Consider delta hedging with volcanic rock
        delta_exposure = position * delta
        if abs(delta_exposure) > self.delta_limit:
            # Add hedging here when needed - this would require a more complex implementation
            pass
