...
class VolcanicRockOptionsStrategy(MarketMakeStrategy):
    ...
    def black_scholes(self, spot_price, strike, time_to_expiry, risk_free_rate, volatility):
        if time_to_expiry <= 0:
            return max(0, spot_price - strike), 1.0
        ...
        N_d1 = nd.cdf(d1)
        N_d2 = nd.cdf(d2)
        return spot_price * N_d1 - strike * math.exp(-risk_free_rate * time_to_expiry) * N_d2
    
    def delta(self, spot_price, strike, time_to_expiry, risk_free_rate, volatility):
        if time_to_expiry <= 0:
            return 0.0
        d1 = (math.log(spot_price / strike) + (risk_free_rate + 0.5 * volatility**2) * time_to_expiry) / (volatility * math.sqrt(time_to_expiry))
        nd = NormalDist(mu=0, sigma=1)
    ...
    def estimate_implied_volatility(self, market_price, spot_price, strike, time_to_expiry, risk_free_rate, precision=0.001):
        if market_price <= 0:
            return 0.01
        vol_low = 0.01
        vol_high = 2.0
        ...
        for _ in range(50):
            price = self.black_scholes(spot_price, strike, time_to_expiry, risk_free_rate, vol_mid)
    ...
    def delta_hedge(self, state: TradingState, order: Order, delta: float) -> dict[str: Order]:
        hedge_orders = {"VOLCANIC_ROCK" : None}
        hedge_quantity = round(order.quantity * delta)
        if "VOLCANIC_ROCK" not in state.order_depths or hedge_quantity == 0:
            return hedge_orders
        rock_depth = state.order_depths["VOLCANIC_ROCK"]
        if hedge_quantity < 0:
            best_ask = min(rock_depth.sell_orders.keys())
            hedge_orders["VOLCANIC_ROCK"] = Order("VOLCANIC_ROCK", best_ask, hedge_quantity)
        elif hedge_quantity > 0:
            best_bid = max(rock_depth.buy_orders.keys())
            hedge_orders["VOLCANIC_ROCK"] = Order("VOLCANIC_ROCK", best_bid, hedge_quantity)
        return hedge_orders
    
    def spot_price(self, state: TradingState) -> float:
        rock_order_depth = state.order_depths["VOLCANIC_ROCK"]
        if len(rock_order_depth.sell_orders) != 0 and len(rock_order_depth.buy_orders) != 0:
            best_ask = min(rock_order_depth.sell_orders.keys())
            best_bid = max(rock_order_depth.buy_orders.keys())
            self.trader_data["VOLCANIC_ROCK_last_price"] = (best_ask + best_bid) / 2
            return (best_ask + best_bid) / 2
        else:
            return self.trader_data.get("VOLCANIC_ROCK_last_price", 10200)
    
    def mid_price(self, state: TradingState) -> float:
        if len(self.order_depth.sell_orders) != 0 and len(self.order_depth.buy_orders) != 0:
            best_ask = min(self.order_depth.sell_orders.keys())
            best_bid = max(self.order_depth.buy_orders.keys())
            self.trader_data[f"{self.symbol}_last_price"] = (best_ask + best_bid) / 2
            return (best_ask + best_bid) / 2
        else:
            return self.trader_data.get(f"{self.symbol}_last_price", 10200)
    
    def option_orders(self, state: TradingState, position: int, buy_order_volume: int, sell_order_volume: int) -> dict[str: Order]:
        spot = self.spot_price(state)
        mid = self.mid_price(state)
        orders = {self.symbol : [], "VOLCANIC_ROCK" : []}
        time_to_expiry = (self.days_to_expiry - state.timestamp / 1000000) / 365.0
        m_t = math.log(self.strike_price / spot) / math.sqrt(time_to_expiry)
        smile_iv = 0.2373 * m_t**2 - 0.0029 * m_t + 0.1492
        implied_vol = self.estimate_implied_volatility(mid, spot, self.strike_price, time_to_expiry, 0)
        iv_diff = implied_vol - smile_iv
        if iv_diff > self.threshold:
            ...
            if len(self.order_depth.sell_orders) > 0:
                ...
                if quantity > 0:
                    ...
                    hedge_orders = self.delta_hedge(state, orders[self.symbol][-1], self.delta(spot, self.strike_price, time_to_expiry, 0, implied_vol))
                    if hedge_orders["VOLCANIC_ROCK"] != None:
                        orders["VOLCANIC_ROCK"].append(hedge_orders["VOLCANIC_ROCK"])
                    buy_order_volume += quantity
            ...
            if len(self.order_depth.buy_orders) > 0:
                ...
                if quantity > 0:
                    ...
                    hedge_orders = self.delta_hedge(state, orders[self.symbol][-1], -self.delta(self.spot_price(state), self.strike_price, time_to_expiry, 0, implied_vol))
                    if hedge_orders["VOLCANIC_ROCK"] != None:
                        orders["VOLCANIC_ROCK"].append(hedge_orders["VOLCANIC_ROCK"])
                    sell_order_volume += quantity
        return orders, buy_order_volume, sell_order_volume
    ...
    def act(self, state: TradingState):
        ...
        buy_order_volume = 0
        sell_order_volume = 0
        option_orders, buy_order_volume, sell_order_volume = self.option_orders(state, position, buy_order_volume, sell_order_volume)
        return option_orders

class Voucher9500Strategy(VolcanicRockOptionsStrategy):
    def __init__(self, symbol: str, limit: int, order_depth: OrderDepth, trader_data):
        super().__init__(symbol, limit, order_depth, trader_data)

class Voucher9750Strategy(VolcanicRockOptionsStrategy):
    def __init__(self, symbol: str, limit: int, order_depth: OrderDepth, trader_data):
        super().__init__(symbol, limit, order_depth, trader_data)

class Voucher10000Strategy(VolcanicRockOptionsStrategy):
    def __init__(self, symbol: str, limit: int, order_depth: OrderDepth, trader_data):
        super().__init__(symbol, limit, order_depth, trader_data)

class Voucher10250Strategy(VolcanicRockOptionsStrategy):
    def __init__(self, symbol: str, limit: int, order_depth: OrderDepth, trader_data):
        super().__init__(symbol, limit, order_depth, trader_data)

class Voucher10500Strategy(VolcanicRockOptionsStrategy):
    def __init__(self, symbol: str, limit: int, order_depth: OrderDepth, trader_data):
        super().__init__(symbol, limit, order_depth, trader_data)

class MagnificentMacaronsStrategy(MarketMakeStrategy):
    """
    CSI-driven market-making + conversion strategy for MAGNIFICENT_MACARONS,
    now with explicit take/clear order logic for better book interaction.
    """
    ...
    def __init__(self, symbol: str, limit: int, order_depth: OrderDepth, trader_data):
        ...
        # CSI / panic parameters
        self.CSI = 45.0
        self.panic_coeff = 0.5
        if f"{symbol}_CSI_history" not in trader_data:
            trader_data[f"{symbol}_CSI_history"] = deque(maxlen=5)

        # Storage cost per unit-long
        self.storage_cost = 0.1

        # Conversion limits
        self.conversion_limit = 10
        self.used_conversions = 0

        # Timestamp counter for printing fair value
        self.timestamp_counter = 0

    def fair_price(self, state: TradingState) -> float | None:
        conv = state.observations.conversionObservations.get(self.symbol)
        if not conv:
            return None
        SI = conv.sunlightIndex
        SP = conv.sugarPrice

        # Record SI
        hist = self.trader_data[f"{self.symbol}_CSI_history"]
        hist.append(SI)

        # Adverse-filtered mid
        bids = [p for p, v in self.order_depth.buy_orders.items() if abs(v) >= self.adverse_volume]
        asks = [p for p, v in self.order_depth.sell_orders.items() if abs(v) >= self.adverse_volume]
        ...
        # Panic bump
        if SI < self.CSI:
            panic = (self.CSI - SI) / self.CSI
            if len(hist) >= 2 and hist[-1] < hist[-2]:
                panic *= 1.2
            return mid + self.panic_coeff * panic * SP
        return mid

def act(self, state: TradingState) -> dict[str, list[Order]]:
    """Generate market‑making orders, conversions, and cross‑market arb."""
    # 0) housekeeping

    self.used_conversions = 0
    pos_local = state.position.get(self.symbol, 0)
    orders: list[Order] = []

    # 1) compute local fair and adjust for storage
    fv_local = self.fair_price(state)
    if fv_local is None:
        return {self.symbol: []}
    if pos_local > 0:
        fv_local -= self.storage_cost * pos_local

    # 2) cross‑market arbitrage
    overseas_sym = f"{self.symbol}_OVERSEAS"
    od_over = state.order_depths.get(overseas_sym)
    conv_over = state.observations.conversionObservations.get(overseas_sym)
    conv_loc  = state.observations.conversionObservations.get(self.symbol)
    fx_rate   = state.observations.plainValueObservations.get("FX_USD_SEASHELL", 1.0)

    if od_over and conv_over and conv_loc:
        # local mid
        bids_l = list(self.order_depth.buy_orders.keys())
        asks_l = list(self.order_depth.sell_orders.keys())
        mid_l  = (max(bids_l) + min(asks_l)) / 2

        # overseas mid (in USD)
        bids_o = list(od_over.buy_orders.keys())
        asks_o = list(od_over.sell_orders.keys())
        mid_o  = (max(bids_o) + min(asks_o)) / 2

        # all‑in adjusted
        cost_in   = conv_over.transportFees + conv_over.importTariff
        cost_out  = conv_loc.transportFees   + conv_loc.exportTariff
        adj_buy_o = mid_o * fx_rate + cost_in
        adj_sell_o= mid_o * fx_rate - cost_out

        # if overseas cheap ⇒ buy overseas / sell local
        if mid_l - adj_sell_o > self.arb_threshold:
            qty = min(self.limit - pos_local, self.limit)
            orders.append(Order(overseas_sym, min(asks_o), qty))
            orders.append(Order(self.symbol,     max(bids_l), -qty))
            # update pos_local so rest of logic sees new position
            pos_local += qty

        # if local cheap ⇒ buy local / sell overseas
        elif adj_buy_o - mid_l > self.arb_threshold:
            qty = min(pos_local + self.limit, self.limit)
            orders.append(Order(self.symbol,     min(asks_l), qty))
            orders.append(Order(overseas_sym,  max(bids_o), -qty))
            pos_local -= qty

    # 3) now your existing CSI + conversion + MM
    #    (use fv_local as fair_value, and pos_local as updated position)
    self.fair_value = fv_local
    conv = conv_loc
    SI = conv.sunlightIndex
    ...
    if SI < self.CSI:
        panic = (self.CSI - SI) / self.CSI
        self.panic_coeff = 0.5 + panic
        self.take_width = max(1, self.take_width - 0.5)
        self.default_edge = max(1, self.default_edge - 1)
        self.conversion_limit = min(self.conversion_limit + 2, self.limit//2)
    else:
        self.panic_coeff = 0.5
        self.take_width = 1
        self.default_edge = 2
    ...
    # conversion logic
    buy_cost  = conv.askPrice + conv.transportFees + conv.importTariff
    sell_rev  = conv.bidPrice - (conv.transportFees + conv.exportTariff)
    if (pos_local + self.used_conversions < self.limit
        and self.used_conversions < self.conversion_limit
        and buy_cost < fv_local - self.take_width):
        qty = min(self.limit - pos_local,
                  self.conversion_limit - self.used_conversions)
        orders.append(Order(self.symbol, int(buy_cost),  qty))
        self.used_conversions += qty
        pos_local += qty

    if (pos_local - self.used_conversions > -self.limit
        and self.used_conversions < self.conversion_limit
        and sell_rev > fv_local + self.take_width):
        qty = min(pos_local + self.limit,
                  self.conversion_limit - self.used_conversions)
        orders.append(Order(self.symbol, int(sell_rev), -qty))
        self.used_conversions += qty
        pos_local -= qty

    # optional short‑on‑overbought
    if SI >= self.CSI and pos_local > -self.limit:
        best_bid = max(self.order_depth.buy_orders.keys(), default=fv_local)
        if best_bid > fv_local + self.take_width:
            qty = min(self.limit + pos_local,
                      self.conversion_limit - self.used_conversions)
            orders.append(Order(self.symbol, best_bid, -qty))
            pos_local -= qty
    ...
    # 4) take / clear / make
    take, b1, s1 = self.take_orders(pos_local, 0, 0)
    pos_local   += b1 - s1
    clear, b2, s2= self.clear_orders(pos_local, b1, s1)
    pos_local   += b2 - s2
    make, _, _  = self.make_orders(pos_local, b1+b2, s1+s2)
    orders.extend(take + clear + make)
...
class Trader:
    ...
    def run(self, state : TradingState) -> tuple[dict[Symbol, list[Order]], int , str]:
        ...
        strategies = {symbol: constructor(symbol, self.limits[symbol], state.order_depths[symbol], trader_data) for symbol, constructor in {
            ...
            "MAGNIFICENT_MACARONS": MagnificentMacaronsStrategy}.items()}
        conversions = 0
        orders = {}
