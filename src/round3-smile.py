...
class BaseVolcanicRockVoucherStrategy(MarketMakeStrategy):
    ...
    def estimate_implied_volatility(self, market_price, spot_price, strike, time_to_expiry, risk_free_rate, precision=0.001, is_call=True):
        ...
        for _ in range(50):
            price = self.black_scholes_call(spot_price, strike, time_to_expiry, risk_free_rate, vol_mid) if is_call else self.black_scholes_put(spot_price, strike, time_to_expiry, risk_free_rate, vol_mid)
    ...
    def fair_price(self, state: TradingState) -> float:
        ...
        if len(self.order_depth.buy_orders) > 0 and len(self.order_depth.sell_orders) > 0:
            ...
            implied_vol = self.estimate_implied_volatility(market_price, spot_price, self.strike_price, time_to_expiry, self.risk_free_rate, is_call=is_call)
            ...
            if time_to_expiry > 0 and spot_price > 0:
                m_t = math.log(self.strike_price / spot_price) / math.sqrt(time_to_expiry)
                smile_iv = 0.2636 * m_t**2 - 0.0003 * m_t + 0.1349
                implied_vol = 0.6 * smile_iv + 0.4 * implied_vol
                self.trader_data[f"{self.symbol}_smile_iv"] = smile_iv
                self.trader_data[f"{self.symbol}_moneyness_log"] = m_t
        ...
        option_price = self.black_scholes_call(spot_price, self.strike_price, time_to_expiry, self.risk_free_rate, self.volatility_estimate) if is_call else self.black_scholes_put(spot_price, self.strike_price, time_to_expiry, self.risk_free_rate, self.volatility_estimate)
    ...
    def act(self, state: TradingState):
        ...
        buy_order_volume = 0
        sell_order_volume = 0
        take, buy_order_volume, sell_order_volume = self.take_orders(position_int, buy_order_volume, sell_order_volume)
        clear, buy_order_volume, sell_order_volume = self.clear_orders(position_int, buy_order_volume, sell_order_volume)
        make, _, _ = self.make_orders(position_int, buy_order_volume, sell_order_volume)
