...
class BaseVolcanicRockVoucherStrategy(MarketMakeStrategy):
    ...
    def update_volatility(self, state):
        ...
        if len(self.trader_data[f"{symbol}_price_history"]) > 5:
            ...
            historical_vol = np.std(returns) * np.sqrt(365)  # Annualized volatility
    ...
    def calculate_implied_volatility(self, state: TradingState) -> float:
        ...
        # Binary search to find implied volatility
        target = market_price
        precision = 0.001
        max_iterations = 100
        
        vol_low = 0.01
        vol_high = 2.0  # 200% vol as upper bound
        
        for i in range(max_iterations):
            vol_mid = (vol_low + vol_high) / 2
            price = black_scholes_call(S, K, T, r, vol_mid)
            
            if abs(price - target) < precision:
                return vol_mid
    ...
    def fair_price(self, state: TradingState) -> float:
        ...
        if self.underlying_price > 0:
            ...
            # Calculate implied volatility to compare with our model
            market_implied_vol = self.calculate_implied_volatility(state)
            if market_implied_vol is not None:
                # Store for tracking
                self.trader_data[f"{self.symbol}_implied_vol"] = market_implied_vol
                
                # If market IV is significantly higher than our model estimate
                if market_implied_vol > self.volatility * 1.15:  # Market overpricing volatility by 15%+
                    # Options overpriced - sell more aggressively
                    self.pricing_thresholds = (0.99, 1.02)
                
                # If market IV is significantly lower than our model estimate
                elif market_implied_vol < self.volatility * 0.85:  # Market underpricing volatility by 15%+
                    # Options underpriced - buy more aggressively
                    self.pricing_thresholds = (0.96, 1.01)
                
                # Market IV close to our estimate - use default thresholds
                else:
                    # Use the default thresholds set in the class constructor
                    pass
    ...
    def act(self, state: TradingState):
        # print("Remaining days: " + str(self.remaining_days))
        self.fair_value = self.fair_price(state)
        position = state.position.get(self.symbol, 0)
        buy_order_volume = 0
        sell_order_volume = 0
        
        # Only trade the vouchers, no hedging with VOLCANIC_ROCK
        orders = {self.symbol: []}
...
class VolcanicRockVoucher10000Strategy(BaseVolcanicRockVoucherStrategy):
    def __init__(self, symbol: str, limit: int, order_depth: OrderDepth, trader_data):
        ...
        self.volatility_adjustment = 0.98  # Reduced from 1.05
        self.pricing_thresholds = (0.985, 1.015)  # Much tighter thresholds
        self.mm_size_factor = 0.18  # Reduced from 0.25
        self.soft_position_limit = int(limit * 0.65)  # More conservative
...
class VolcanicRockStrategy(MarketMakeStrategy):
    ...
    def update_volatility_skew(self):
        """Track and analyze volatility skew across strike prices"""
        strikes = [9500, 9750, 10000, 10250, 10500]
        implied_vols = []
        
        # Collect implied vols for each strike
        for strike in strikes:
            symbol = f"VOLCANIC_ROCK_VOUCHER_{strike}"
            iv = self.trader_data.get(f"{symbol}_implied_vol", None)
            if iv:
                implied_vols.append((strike, iv))
        
        # Need at least 3 points to analyze skew
        if len(implied_vols) >= 3:
            # Sort by strike
            implied_vols.sort(key=lambda x: x[0])
            
            # Calculate skew (simplified)
            low_strike_iv = implied_vols[0][1]
            high_strike_iv = implied_vols[-1][1]
            skew = (high_strike_iv - low_strike_iv) / low_strike_iv
    ...
    def fair_price(self, state: TradingState) -> float:
        ...
        if len(self.order_depth.sell_orders) != 0 and len(self.order_depth.buy_orders) != 0:
            ...
            # Analyze the volatility skew across options (if available)
            self.update_volatility_skew()
    ...
    def act(self, state: TradingState):
        ...
        # Since we're no longer delta hedging, just use standard market making
        buy_order_volume = 0
        sell_order_volume = 0
        
        # Check for any significant directional signals from volatility skew
        skew = self.trader_data.get("vol_skew", 0)
        if abs(skew) > 0.2:
            # Strong skew indicates directional bias
            if skew > 0:  # Positive skew suggests downside risk
                # Be more conservative with buying, more aggressive with selling
                self.take_width = 3  # Wider threshold for buying
                self.default_edge = 3  # Wider spread
            else:  # Negative skew suggests upside potential
                # Be more aggressive with buying
                self.take_width = 1  # Narrower threshold for buying
...
class Trader:
    ...
    def run(self, state : TradingState) -> tuple[dict[Symbol, list[Order]], int , str]:
        ...
        # # stuff only relavent when we are backtest over day 1, 2, AND 3. Comment out and hardcode to 5 for submission
        # print("Timestamp: " + str(state.timestamp))
        # global current_day
        # if state.timestamp == 0:
        #     current_day -= 1  # Days are 1-indexed
        # trader_data["current_round"] = current_day
        # print("Current round: " + str(current_day))
