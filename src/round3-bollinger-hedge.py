...
class BaseVolcanicRockVoucherStrategy(MarketMakeStrategy):
    ...
    def act(self, state: TradingState):
        ...
        orders = take + clear + make
        hedge_orders = []

        # Delta hedging logic
        delta = self.trader_data.get(f"{self.symbol}_delta", 0.5)
        delta_exposure = position * delta
        net_delta = delta_exposure + rock_position

        if abs(net_delta) > self.delta_limit:
            # Determine hedge quantity (reduce net delta to 0)
            hedge_qty = int(-net_delta * self.hedge_ratio)
            rock_depth = state.order_depths.get("VOLCANIC_ROCK", None)

            if rock_depth:
                # if hedge_qty > 0:
                #     # Buy VOLCANIC_ROCK to hedge
                #     best_ask = min(rock_depth.sell_orders.keys(), default=None)
                #     if best_ask is not None:
                #         max_qty = -rock_depth.sell_orders[best_ask]
                #         qty = min(hedge_qty, max_qty)
                #         if qty > 0:
                #             hedge_orders.append(Order("VOLCANIC_ROCK", best_ask, qty))
                # elif hedge_qty < 0:
                if hedge_qty < 0:
                    # Sell VOLCANIC_ROCK to hedge
                    best_bid = max(rock_depth.buy_orders.keys(), default=None)
                    if best_bid is not None:
                        max_qty = rock_depth.buy_orders[best_bid]
                        qty = min(-hedge_qty, max_qty)
                        if qty > 0:
                            hedge_orders.append(Order("VOLCANIC_ROCK", best_bid, -qty))

        return {self.symbol: orders, "VOLCANIC_ROCK": hedge_orders} if hedge_orders else {self.symbol: orders}
