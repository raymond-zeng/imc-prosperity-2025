...
#     CSI_WINDOW = 20  # lookback window for critical sunlight index
#     PANIC_PREMIUM = 0.15  # 15% premium when in panic

#     def __init__(self, symbol: str, limit: int, order_depth: OrderDepth, trader_data):
#         super().__init__(symbol, limit, order_depth, trader_data)
#         # track recent sunlight indices
#         self.sunlight_history = deque(maxlen=self.CSI_WINDOW)
#         # fair value baseline is sugar price
#         self.fair_value = 0

#     def fair_price(self, state: TradingState) -> float:
#         # extract conversion observation
...
#         # compute CSI once enough data
#         if len(self.sunlight_history) == self.CSI_WINDOW:
#             mean = sum(self.sunlight_history) / len(self.sunlight_history)
#             std = (sum((x - mean)**2 for x in self.sunlight_history) / len(self.sunlight_history))**0.5
#             CSI = mean - std
#         else:
#             CSI = min(self.sunlight_history)
#         # panic pricing if sunlight below CSI
#         if sunlight < CSI:
#             fair = sugar * (1 + self.PANIC_PREMIUM)
#         else:
#             fair = sugar
#         return fair

#     def act(self, state: TradingState):
#         # determine theoretical fair price
#         fv = self.fair_price(state)
#         if fv is None:
#             return {self.symbol: []}
#         self.fair_value = fv
#         position = state.position.get(self.symbol, 0)
#         buy_vol, sell_vol = 0, 0
#         # use simple market making around fair value
#         # place one layer on each side
#         bid = round(fv - 1)
#         ask = round(fv + 1)
#         orders = []
#         # buy side
#         if len(self.order_depth.sell_orders) > 0 and position < self.limit:
#             qty = min(-self.order_depth.sell_orders[min(self.order_depth.sell_orders)],
#                       self.limit - position)
#             if qty > 0:
#                 orders.append(Order(self.symbol, bid, qty))
#                 buy_vol += qty
#         # sell side
#         if len(self.order_depth.buy_orders) > 0 and position > -self.limit:
#             qty = min(self.order_depth.buy_orders[max(self.order_depth.buy_orders)],
#                       self.limit + position)
#             if qty > 0:
...
class MagnificentMacaronsStrategy(MarketMakeStrategy):
    CSI_WINDOW = 20         # Lookback in timesteps for Critical Sunlight Index
    PANIC_PREMIUM = 0.15    # 15% panic markup when sunlight < CSI

    def __init__(self, symbol: str, limit: int, order_depth: OrderDepth, trader_data):
        super().__init__(symbol, limit, order_depth, trader_data)
        self.sunlight_history = deque(maxlen=self.CSI_WINDOW)
        # Soft limit factor (e.g., 80% of hard limit)
        self.soft_limit = int(self.limit * 0.8)
        # Minimum order fraction when at soft limit
        self.min_order_frac = 0.2
    ...
    def fair_price(self, state: TradingState) -> float:
        ...
        self.sunlight_history.append(sunlight)
        # Compute CSI when enough data
        if len(self.sunlight_history) >= self.CSI_WINDOW:
            mean = sum(self.sunlight_history) / len(self.sunlight_history)
            var  = sum((x - mean)**2 for x in self.sunlight_history) / len(self.sunlight_history)
            std  = var**0.5
            CSI  = mean - std
        else:
            CSI = min(self.sunlight_history)
        ...
        # Panic pricing
        return sugar * (1 + self.PANIC_PREMIUM) if sunlight < CSI else sugar

    def _scale_qty(self, base_qty: int, position: int) -> int:
        # Scale order quantity as position nears soft limit
        ratio = abs(position) / max(self.soft_limit, 1)
        scale = max(self.min_order_frac, 1 - ratio)
    ...
    def act(self, state: TradingState):
        ...
        # Compute one-unit base quotes
        bid = int(fair - 1)
        ask = int(fair + 1)
        base_qty = 1
        qty = self._scale_qty(base_qty, pos)

        # Market take if attractive
        if depth.sell_orders and pos < self.limit:
            best_ask = min(depth.sell_orders)
            avail    = -depth.sell_orders[best_ask]
            take_qty = min(avail, self.limit - pos)
            scaled   = self._scale_qty(take_qty, pos)
            if best_ask <= bid and scaled > 0:
                orders.append(Order(self.symbol, best_ask, scaled))
                pos += scaled

        if depth.buy_orders and pos > -self.limit:
            best_bid = max(depth.buy_orders)
            avail    = depth.buy_orders[best_bid]
            take_qty = min(avail, self.limit + pos)
            scaled   = self._scale_qty(take_qty, pos)
            if best_bid >= ask and scaled > 0:
                orders.append(Order(self.symbol, best_bid, -scaled))
                pos -= scaled

        # Soft-limited market making around fair
        if depth.sell_orders and pos < self.limit:
            orders.append(Order(self.symbol, bid, qty))
            pos += qty
        if depth.buy_orders and pos > -self.limit:
