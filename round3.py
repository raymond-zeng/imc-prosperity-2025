...
class BaseVolcanicRockVoucherStrategy(MarketMakeStrategy):
    def __init__(self, symbol: str, limit: int, order_depth: OrderDepth, trader_data):
        ...
        self.volatility_window = trader_data.get(f"{symbol}_volatility_window", 10)  # Days for rolling window
    ...
    def fair_price(self, state: TradingState):

        if len(self.order_depth.buy_orders) > 0 and len(self.order_depth.sell_orders) > 0:
            best_bid = max(self.order_depth.buy_orders.keys())
            best_ask = min(self.order_depth.sell_orders.keys())
            fair = (best_bid + best_ask) / 2
            self.trader_data[f"{self.symbol}_fair_price"] = fair
            return fair
        else:
            return self.trader_data.get(f"{self.symbol}_fair_price", None)
    ...
    def rock_hedge_orders(
        ...
        position = self.state.position.get(self.symbol, 0)

        if orders == None or len(orders) == 0:
            post_trade_position = position
        else:
            post_trade_position = position + sum(
                order.quantity for order in orders
            )

        target_rock_position = -delta * post_trade_position
    ...
    def voucher_orders(self, state: TradingState):
        position = state.position.get(self.symbol, 0)
        self.traderData[f"{self.symbol}_past_volatility"].append(self.volatility)

        if (
            len(self.traderData[f"{self.symbol}_past_volatility"])
            < self.std_window
        ):
            return None, None
        ...
        if (
            ...
            self.traderData[f"{self.symbol}_past_volatility"].pop(0)

        z_score = (
            self.volatility - self.mean_volatility
        ) / np.std(self.traderData[f"{self.symbol}_past_volatility"])
        ...
        if z_score >= self.threshold:
            if position != -self.limit:
                ...
                if len(self.order_depth.buy_orders) > 0:
                    ...
                    quantity = min(
                        target_quantity,
                        abs(self.order_depth.buy_orders[best_bid]),
                    )
                    quote_quantity = target_quantity - quantity
                    if quote_quantity == 0:
                        return [Order(self.symbol, best_bid, -quantity)], []
                    else:
                        return [Order(self.symbol, best_bid, -quantity)], [
                            Order(self.symbol, best_bid, -quote_quantity)
                        ]
            ...
            if position != self.limit:
                ...
                if len(self.order_depth.sell_orders) > 0:
                    ...
                    quantity = min(
                        target_quantity,
                        abs(self.order_depth.sell_orders[best_ask]),
                    )
                    quote_quantity = target_quantity - quantity
                    if quote_quantity == 0:
                        return [Order(self.symbol, best_ask, quantity)], []
                    else:
                        return [Order(self.symbol, best_ask, quantity)], [
                            Order(self.symbol, best_ask, quote_quantity)
                        ]
        ...
        def delta_hedge_coconut_coupon_orders(
            ...
            if target_rock_quantity > 0:
                ...
                quantity = min(
                    quantity,
                    self.self - (rock_position + rock_buy_orders),
                )
                if quantity > 0:
        ...
        def act(self, state: TradingState):
            fair_price = self.fair_price(state)
            tte = self.remaining_days / 365.0

            rock_order_depth = state.order_depths["VOLCANIC_ROCK"]
            volatility = BlackScholes.implied_volatility(
                fair_price,
                self.trader_data["VOLCANIC_ROCK_mid_price"],
                self.strike,
                tte,
            )
            ...
            rock_orders = self.rock_hedge_orders(
                rock_order_depth,
                take_orders,
                state.position("VOLCANIC_ROCK", 0),
                state.position(self.symbol, 0),
                delta,
            )

            return take_orders + make_orders + rock_orders
